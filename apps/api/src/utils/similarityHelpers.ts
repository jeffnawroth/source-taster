...
export const DATE_WEIGHTS = { year: 0.8, month: 0.15, day: 0.05 } as const
export const NAME_WEIGHTS = { family: 0.85, given: 0.15 } as const

// Month names (English/German) used for parsing string dates
...
export const MONTH_REGEX = new RegExp(`\\b(${Object.keys(MONTH_MAP).join('|')})\\b`, 'i')
...
export function parseDateFromCSL(value: any): NormalizedDateParts | null {
...
export function parseDateFromString(input: string): NormalizedDateParts | null {
...
export function extractDateParts(value: unknown): NormalizedDateParts | null {
...
export function dateSimilarity(a: unknown, b: unknown): number | null {
...
export function isCSLNameObject(x: any): boolean {
  return x && typeof x === 'object' && ('family' in x || 'given' in x || 'literal' in x)
}

export function tokenizeGiven(given: string): string[] {
  const raw = given
    .replace(/\./g, ' ')
    .replace(/-/g, ' ')
    .split(/\s+/)
...
export function initialsFromTokens(tokens: string[]): string {
...
export function parseName(
...
): NormalizedNameParts | null {
  if (!value)
    return null

  if (isCSLNameObject(value)) {
    const obj = value as any
    const family = obj.family ? normalizeValue(String(obj.family), rules) : null
    const givenStr = obj.given ? normalizeValue(String(obj.given), rules) : null
  ...
  if (typeof value === 'string') {
    const s = normalizeValue(value, rules).trim()
    if (!s)
      return null
...
export function isSubsequence(shorter: string, longer: string): boolean {
  let i = 0
...
export function nameSimilarity(
  ...
  const na = parseName(a, rules, normalizeValue)
  const nb = parseName(b, rules, normalizeValue)
  ...
  if (famA === famB) {
    ...
    // Fuzzy allowance for minor typos/transliterations
    const sim = levenshtein(famA, famB).similarity
    if (sim >= 0.9)

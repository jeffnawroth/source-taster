...
- **Branch protection is tightened in two steps, deliberately.** Before this
  ...
  **Known follow-up, not addressed here:** `gh pr merge --squash` without
  `--auto` will fail if required checks are still pending when the release job
  reaches it. Switching that call to `--auto` is the proportionate fix, but
  `release.yml` is human-authorized-only under `AGENTS.md` and deploys to
  production, so it is left for a separate, explicitly approved change.

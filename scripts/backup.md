...
- Ablage: `/srv/backups/source-taster/` (Postgres-Custom-Dump + Keystore-Tar, Rotation 14 Tage).
...
- Offsite: `BACKUP_RSYNC_TARGET` im Cron-Kontext setzen, um eine zweite Kopie extern zu halten.
  Alternativ manuell: `scp -r /srv/backups/source-taster user@host:~/backups`
...
docker compose exec -T postgres pg_restore --no-owner --clean --if-exists \
  -U sourcetaster -d sourcetaster < /srv/backups/source-taster/postgres-<stamp>.dump
```
...
tar -xzf /srv/backups/source-taster/keystore-<stamp>.tar.gz -C apps/api
